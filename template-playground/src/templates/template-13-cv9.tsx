...
export const CV9Template: React.FC<UniversalTemplateProps> = ({ data }) => (
  <div style={{
    width: '850px',
    minHeight: '1200px',
    background: '#ffffff',
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span>{data.personal.fullName.split(' ')[0]?.[0] || 'F'}</span>
          <span>{data.personal.fullName.split(' ')[1]?.[0] || 'L'}</span>
        </div>
        {/* Diagonal X lines */}
          ...
          <div style={{ background: 'white', padding: '18px 15px', marginBottom: '15px' }}>
            ...
            {data.education.map((edu, i) => (
              <div key={i} style={{ fontSize: '10px', color: '#333', lineHeight: 1.6, marginBottom: '8px' }}>
                <p style={{ marginBottom: '2px' }}><strong>{edu.degree}{edu.field && ` in ${edu.field}`}</strong></p>
                <p>{edu.institution}</p>
              </div>
            ))}
          </div>
